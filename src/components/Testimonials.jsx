...
function Card({ t }) {
  return (
    ...
    >
      ...
      <p style={{ fontFamily: 'Inter', fontSize: '14px', color: 'rgba(255,255,255,0.65)', lineHeight: 1.7, marginBottom: '20px' }}>"{t.text}"</p>
      <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
        <div style={{ fontFamily: 'Syne', fontSize: '14px', fontWeight: 700, color: '#fff' }}>{t.name}</div>
        <div style={{ fontFamily: 'Inter', fontSize: '12px', color: 'rgba(255,255,255,0.35)' }}>{t.car}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <section style={{ padding: '120px 0', background: 'var(--color-bg-primary)', overflow: 'hidden' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 48px', marginBottom: '56px' }}>
        ...
        <h2 style={{ fontFamily: 'Bebas Neue', fontSize: 'clamp(48px, 7vw, 96px)', color: '#fff', lineHeight: 0.95 }}>WHAT OUR CLIENTS SAY.</h2>
      </div>
